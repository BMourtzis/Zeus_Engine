...
impl<B: Backend> CameraState<B> {
    pub fn new(
        ...
        for _i in 0..size {
            buffers.push(Some(BufferState::new_uniform_buffer::<UniformBufferObject>(
                Rc::clone(&device),
                1,
                memory_types
            )));
    ...
    pub fn update_ubo(&mut self, model: Matrix4, view: Matrix4, proj: Matrix4) {
    ...
    pub fn update_model(&mut self, update: Matrix4) {
    ...
    pub fn update_view(&mut self, update: Matrix4) {
...
impl<B:Backend> Drop for CameraState<B> {
    fn drop(&mut self) {
        ...
        unsafe {
            self.device.borrow()
                .device.destroy_descriptor_pool(self.camera_desc_pool.take().unwrap());
        }
    }
}