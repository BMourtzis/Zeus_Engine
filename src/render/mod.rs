...
pub fn render() {
    ...
    let (_window, instance, mut adapters, surface) = {
        ...
        let instance = back::Instance::create("gfx-rs quad", 1)
            .expect("Failed to create an instace!");
        let surface = unsafe {
...
where B: gfx_hal::Backend
{
    fn new(
        ...
        //Build new device and associated command queues
        let family = adapter
            .queue_families
            .iter()
            .find(|family| {
        ...
        let padded_uploade_size = ((upload_size + non_coherent_alignment - 1) / non_coherent_alignment) * non_coherent_alignment;

        let mut image_upload_buffer = ManuallyDrop::new(unsafe {
            device.create_buffer(padded_uploade_size, buffer::Usage::TRANSFER_SRC)
        }.unwrap());
        ...
        let image_upload_memory = unsafe {
            ...
            let mapping = device.map_memory(&memory, 0..padded_uploade_size).unwrap();
            ...
            device.flush_mapped_memory_ranges(iter::once((&memory, 0..padded_uploade_size))).unwrap();
            device.unmap_memory(&memory);
            ManuallyDrop::new(memory)
        };
        ...
        let copy_fence = device.create_fence(false).expect("Can't create fence");
        unsafe {
        ...
        //Define the maximum number of frames we want to be able to be "in flight" (being computed simultaneously) at once
        ...
        let pipeline_layout = ManuallyDrop::new(
            ...
            }.expect("cant' create pipeline layout")
        );
        let pipeline = {
            let vs_module = {
            ...
            let fs_module = {
                let spirv = pso::read_spirv(Cursor::new(&include_bytes!("../../data/quad.frag.spv")[..]))
                    .unwrap();
                unsafe {
                    device.create_shader_module(&spirv)
                }.unwrap()
...
where B: gfx_hal::Backend
{
    fn drop(&mut self) {
        ...
        unsafe {
            ...
            self.device.destroy_buffer(ManuallyDrop::into_inner(ptr::read(&self.image_upload_buffer)));
            ...
            self.device.destroy_graphics_pipeline(ManuallyDrop::into_inner(ptr::read(&self.pipeline)));
            self.device.destroy_pipeline_layout(ManuallyDrop::into_inner(ptr::read(&self.pipeline_layout)));
